...
def get_mapping_fields_command(service):
    ...
    search_offset = get_integration_context().get('offset', 0)
...
def get_fields_query_part(notable, prefix, fields):
    ...
    Args:
        ...
        fields: The fields to search in the notable for.
...
def get_drilldown_timeframe(saved_search, data):
    ...
    Args:
        ...
        data (list): The raw data from the notable
...
def get_time_data(raw_data):
    """ Gets the data related to time from the raw data

    Args:
        raw_data: The raw data

    Returns: Two time values, info_min_time & info_max_time
...
def identity_enrichment(service, notable, enrichment, num_enrichment_events):
    ...
    if not task_status:
        ...
        demisto.info("No users were found in notable. Failed submitting identity enrichment request to Splunk for "
                     "enrichment {}".format(eid))
    return task_status
...
def asset_enrichment(service, notable, enrichment, num_enrichment_events):
    ...
    if not task_status:
        ...
        demisto.info("No assets were found in notable. Failed submitting asset enrichment request to Splunk for "
                     "enrichment {}".format(eid))
    return task_status
...
def is_enrichment_exceeding_timeout(enrichment, enrichment_timeout):
    ...
    Returns: True if the enrichment exceed the given timeout, False otherwise
...
def reset_enriching_fetch_mechanism():
    """ Resets all the fields regarding the enriching fetch mechanism. """

    integration_content = get_integration_context()
    for field in (ENRICHMENTS, LAST_RUN_REGULAR_FETCH, LAST_RUN_OVER_FETCH, NUM_FETCHED_INCIDENTS,
                  NOT_YET_ENRICHED_NOTABLES):
        if field in integration_content:
            del integration_content[field]
    set_integration_context(integration_content)
